...
Original file is located at
    https://colab.research.google.com/drive/1K0eWAvXKhF-gzIwAGZD3VL3Oj9xNhysv
...
with open('data.log', 'w') as g:
  ...
  for line in lines:
    ...
    try:
      rate = float(rate)    
      defs[A].append([B, rate])

    except ValueError:
      pass 

  for player in defs:
    top_n = sorted(defs[player], key=lambda v:v[1], reverse=False)[0:1]
    print(player, top_n)
...
with open('data.log', 'w') as g:
  for player in final_zone_dict:
    g.write('%s\t%s' % (player, final_zone_dict[player]))
    g.write('\n')


#current error: keys cannot be lists bc they are mutable.
